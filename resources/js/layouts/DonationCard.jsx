...
export default function DonationCard({ kegiatan }) {
    ...
    const isExpired = !!(open_donasi && daysLeft <= 0);

    return (
        <div className={`bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 
        ${status_post === 'hide' ? 'hidden' : ''}`}>
            <Link
                href={isExpired ? '#' : `/donasi/${id}`}
                className={`${isExpired ? 'cursor-default' : 'block group cursor-pointer'}`}
            ...
            >
                ...
                <div className="p-4">
                    ...
                    {/* Tampilan Status Donasi (Visual Button) */}
                    {isExpired ? (
                        <div
                            className="block w-full text-center py-2 px-4 rounded-md 
                            text-white font-semibold bg-gray-400 cursor-not-allowed"
                        >
                            Donasi Telah Berakhir
                        </div>
                    ):(
                        <div
                            className="block w-full text-center py-2 px-4 rounded-md bg-primary-green text-white font-semibold group-hover:bg-green-700 transition-colors duration-200"
                        >
                            Donasi Sekarang
                        </div>
                    )}
